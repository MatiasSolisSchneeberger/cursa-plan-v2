...
const ToggleMateria = ({ materia, mesas, planDeEstudio }) => {
    ...
    const obtenerProximaFecha = (fechas) => {
        ...
        const fechasFuturas = fechas.filter(
            (fecha) => new Date(fecha) > new Date(),
        );

        // Si hay fechas futuras, encontramos la más cercana
        if (fechasFuturas.length > 0) {
            const proximaFecha = fechasFuturas.reduce(
                (fechaMasCercana, fechaActual) => {
                    const diferenciaFechaMasCercana = Math.abs(
                        new Date(fechaMasCercana) - new Date(),
                    );
                    const diferenciaFechaActual = Math.abs(
                        new Date(fechaActual) - new Date(),
                    );

                    // Comparamos cuál de las dos fechas es más cercana
                    return diferenciaFechaActual < diferenciaFechaMasCercana
                        ? fechaActual
                        : fechaMasCercana;
                },
            );
                ...
                <footer className="flex w-full flex-col sm:flex-row gap-2 p-2">
                    ...
                    <section className="md:w-full md:flex-1 outline outline-outline rounded-xl p-2 flex flex-col gap-2.5 items-center justify-start self-stretch shrink-0 relative overflow-hidden">
                            ...
                            {mesas?.length > 0 ? (
                                <>
                                    ...
                                    <p className="text-center text-title-small flex-1 relative flex items-center justify-center">
                                        {obtenerProximaFecha(mesas)
                                            ? formatDate(obtenerProximaFecha(mesas))
                        ...
                        <div className="flex flex-wrap gap-2.5 w-full">
                            ...
                            >
                                ...
                                {/* Contenedor del dialog con animación */}
                                <div
                                    class="fixed inset-0 bg-black dialog-backdrop dialog-backdrop.hidden opacity-25 ease-in-out transition-all duration-300"
                                    onclick="closeDialog('dialog-1')"
                                />
                                <div class="relative bg-white w-max max-w-lg rounded-lg shadow-lg overflow-hidden transform transition-all duration-300 scale-95 opacity-0">
                                    <div class="flex flex-col items-center p-6">
                                        ...
                                        {/* Contenido */}
                                        <div class="text-gray-700 mb-4 text-center">
                    ...
                    <section className="md:w-full md:flex-1 border border-outline rounded-xl p-2 flex flex-col gap-2.5 items-center justify-start self-stretch shrink-0 relative overflow-hidden">
                        ...
                        <footer className="flex flex-wrap gap-2.5 items-start justify-start self-stretch shrink-0 relative">
                            ...
                            <md-elevated-button
                                disabled={!planDeEstudio}
                                className="w-1/2 flex-1 sm:w-full"
                            >
                                Descargar
                            </md-elevated-button>
                        </footer>
